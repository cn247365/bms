...
/// </summary>
    public class BookPictureService : Service< BookPicture >, IBookPictureService
    {
        private readonly IRepositoryAsync<BookPicture> repository;
		private readonly IDataTableImportMappingService mappingservice;
        private readonly NLog.ILogger logger;
        public  BookPictureService(
          IRepositoryAsync< BookPicture> repository,
            ...
            : base(repository)
        {
            ...
            this.logger = logger;
                 ...
                 public async  Task<IEnumerable<BookPicture>> GetByBookId(int  bookid) => await repository.GetByBookId(bookid);
                ...
                public async Task ImportDataTable(DataTable datatable,string username)
        {
            var mapping = await this.mappingservice.Queryable()
                              .Where(x => x.EntitySetName == "BookPicture" && 
                                 (x.IsEnabled == true  || (x.IsEnabled == false &&  x.DefaultValue != null))
                                 ).ToListAsync();
            if (mapping.Count == 0)
            {
                throw new KeyNotFoundException("没有找到BookPicture对象的Excel导入配置信息，请执行[系统管理/Excel导入配置]");
            }
            foreach (DataRow row in datatable.Rows)
            {
                ...
                {
                    var item = new BookPicture();
                    var bookpicturetype = item.GetType();
                    foreach (var field in mapping)
                    {
						...
						{
							var propertyInfo = bookpicturetype.GetProperty(field.FieldName);
                                                        //关联外键查询获取Id
				...
				public async Task<Stream> ExportExcel(string filterRules = "",string sort = "Id", string order = "asc")
        {
            ...
            var bookpictures  = await this.Query(new BookPictureQuery().Withfilter(filters)).Include(p => p.Book).OrderBy(n=>n.OrderBy(sort,order)).SelectAsync();
            
            var datarows = bookpictures .Select(  n => new { 
    ...
    Description = n.Description
            ...
            return await NPOIHelper.ExportExcelAsync("Book Picture", datarows,expcolopts);
        }
