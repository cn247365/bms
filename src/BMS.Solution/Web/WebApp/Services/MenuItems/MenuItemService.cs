...
namespace WebApp.Services
{
  public class MenuItemService : Service<MenuItem>, IMenuItemService
  {
    ...
    public async Task<Stream> ExportExcelAsync(string filterRules = "", string sort = "Id", string order = "asc")
    {
      ...
      return await NPOIHelper.ExportExcelAsync("系统菜单", datarows);

    }

    public async Task CreateWithControllerAsync()
    {
      ...
      var controlleractionlist = asm.GetTypes()
              ...
              &&
              m.GetCustomAttributes().Where(y => y.GetType() == typeof(RouteAttribute)).Any()
              )
              .Select(x => new {
                ...
                Route =
                x.GetCustomAttributes().Where(y => y.GetType() == typeof(RouteAttribute)).Select(rx=>( RouteAttribute )rx).FirstOrDefault() })
              .OrderBy(x => x.Controller).ThenBy(x => x.Action).ToList();
      var i = 1000;
      foreach (var item in controlleractionlist.OrderBy(x=>x.Controller).ThenBy(x=>x.Action))
      {
        var routename = item.Route.Name;
        if (!string.IsNullOrEmpty(routename))
        {
          var menu = new MenuItem
          {
            ...
            Title = routename,
            Url = "/" + item.Controller.Replace("Controller", "") + "/" + item.Route.Template,
            Action = item.Route.Template,
    ...
    public async Task ReBuildMenusAsync()
    {
      foreach (var item in this.Queryable().ToList())
      {
        this.Delete(item);
      }
      await this.CreateWithControllerAsync();
    }
  }
}
