...
namespace WebApp.SmartHubs
{
  ...
  public class ChatHub : Hub
  {
    ...
    public async Task Connect()
    {
      ...
      var givenName= user.Claims.FirstOrDefault(x => x.Type == "GivenName")?.Value;
      ...
      givenName= string.IsNullOrEmpty(givenName) ? userName : givenName;
      if (connectuser==null)
      {
        ...
        // send to caller
        this.Clients.Caller.onConnected(connectionId, userName, givenName, ConnectedUsers, CurrentMessage);

        // send to all except caller client
        this.Clients.AllExcept(connectionId).onNewUserConnected(connectionId, userName, givenName, UserImg, logintime);
      }
      else
      {
        ...
        // send to caller
        this.Clients.Client(connectionId).onConnected(connectionId, userName, givenName, ConnectedUsers, CurrentMessage);
    ...
    public void SendMessageToAll(string userName, string givenName, string message, string time,string avatar)
    {
      var UserImg = avatar;
      // store last 100 messages in cache
      this.AddMessageinCache(userName, givenName, message, time, UserImg);

      // Broad cast message
      this.Clients.All.messageReceived(userName, givenName, message, time, UserImg);

    }

    private void AddMessageinCache(string userName,string givenName, string message, string time, string UserImg)
    {
      CurrentMessage.Add(new Messages { UserName = userName,GivenName= givenName, Message = message, Time = time, UserImage = UserImg });
    ...
    public void SendPrivateMessage(string toUserId, string message)
    {
      ...
      if (toUser != null && fromUser != null)
      {
        ...
        {
          this.Clients.Client(toid).sendPrivateMessage(fromUserId, fromUser.GivenName, toUserId, toUser.GivenName, message, UserImg, CurrentDateTime);
        }
        

        // send to caller user
        this.Clients.Caller.sendPrivateMessage(fromUserId, fromUser.GivenName, toUserId, toUser.GivenName, message, UserImg, CurrentDateTime);
      }
