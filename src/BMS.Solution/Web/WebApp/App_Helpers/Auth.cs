...
namespace WebApp
{
  public static class Auth
  {
    ...
    public static string GetGivenName(string username = null) => cache.GetOrAdd($"Identity.GivenName.{username ?? HttpContext.Current.User.Identity.Name}", () =>
    {
      username = string.IsNullOrEmpty(username) ? HttpContext.Current.User.Identity.Name : username;
      var userid = db.Ado.GetString("select [GivenName] from [dbo].[AspNetUsers] where [username]=@username", new { username });
      return userid;
    });
